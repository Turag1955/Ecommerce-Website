...
function sendmessage() {
    ...
    if (name == "") {
        alert('name is reqruid');
    } else if (email == "") {
        alert('email is reqruid');
    } else if (mobile == "") {
        alert('mobile is reqruid');
    } else if (comment == "") {
        alert('comment is reqruid');
    } else {
        jQuery.ajax({
            url: 'sendmessage.php',
            type: 'post',
...
function register_submit() {
    ...
    var is_error = "";
    if (name == "") {
        jQuery('#name_error').html("Name feild is requred");
        is_error = 'yes';
    }
    if (email == "") {
        jQuery('#email_error').html("email feild is requred");
        is_error = 'yes';
    }
    if (mobile == "") {
        jQuery('#mobile_error').html("mobile feild is requred");
        is_error = 'yes';
    }
    if (password == "") {
        jQuery('#password_error').html("password feild is requred");
        is_error = 'yes';
    }
    if (is_error == '') {
        jQuery.ajax({
            ...
            success: function (result) {
                if (result == 'email') {
                    jQuery('#email_error').html('This email All ready Exits');
                }
                if (result == 'insert') {
                    jQuery('.register p').html('Register Success');
                }
...
function login() {
    ...
    var is_error = '';
    if (email == "") {
        jQuery('#login_email_error').html("email feild is requred");
        is_error = 'yes';
    }
    if (password == "") {
        jQuery('#login_password_error').html("password feild is requred");
        is_error = 'yes';
    }
    ...
    if (is_error == '') {
        jQuery.ajax({
            ...
            success: function (result) {
                ...
                if (result == 'invalid') {
                    jQuery('.login p').html('login invalid');
                }
...
function add_to_cat(pid, type, qtyby) {
    if (type == 'update') {
...
function product_soting(cat_id) {
    var soting_select = jQuery('#soting_select').val();
    window.location.href = "category.php?id=" + cat_id + "&sot=" + soting_select;

}
function wishlist_manage(pid, remove) {
    jQuery.ajax({
        url: 'wishlist_manage.php',
        type: 'post',
        data: 'pid=' + pid + '&remove=' + remove,
        success: function (result) {
            if (result == 'not_login') {
            ...
            if (result == 'wishlist exit') {
                alert('Wish list All Ready exist..!');
            }
        }
    });
}
...
function email_otp_varify() {

    var otp = jQuery('#otp').val();
    if (otp == '') {
        jQuery('#otpl_error').html("OTP is Required ");
    } else {
        jQuery('#otpl_error').hide();
        jQuery('.email_otp_varify').attr('disabled', true);
        jQuery('.email_otp_varify').hide();
        jQuery('.email_verypied').show();
    }
}
...
function change_password() {
    ...
    if (current_password == '') {
        jQuery('#current_password_error').html("Current Password is Required..!");
        is_error = 'yes';
    }
    if (new_password == '') {
        jQuery('#new_password_error').html("New Password is Required..!");
        is_error = 'yes';
    }
    if (confirm_password == '') {
        jQuery('#confirm_password_error').html("Confirm Password is Required..!");
        is_error = 'yes';
    }
    if (new_password != confirm_password && new_password && confirm_password != '') {
        jQuery('#confirm_password_error').html("Confirm Password Not match");
        is_error = 'yes';
    }
    if (is_error == '') {
        jQuery.ajax({
...
function forgot_password() {
    ...
    if (email == '') {
        jQuery('#email_error').html("New email is Required..!");
        is_error = 'yes';
    }
    ...
    if (is_error == '') {
        jQuery.ajax({
            ...
            success: function (result) {
                ...
                jQuery('#result_error').html(result);
            }
        });
    }
...
function set_coupon() {
    ...
    if (coupon_name == '') {
        jQuery('#coupon_error').html('Feild is required');
    } else {
        jQuery.ajax({
            url: 'set_coupon.php',
            type: 'post',
            ...
            success: function (result) {
               ...
               if(data.is_error == 'yes'){
                   jQuery('.coupon-table').hide();
                   jQuery('#coupon_error').html(data.result);
               }
               if(data.is_error == 'no'){
                   jQuery('.coupon-table').show();
                   jQuery('#discount_less').html(data.less_discount);
                   jQuery('#discount_total').html(data.discount_price);
               }
